...
function install_fonts() {
  brew tap homebrew/linux-fonts > /dev/null 2>&1
...
function setup_zsh() {
  ...
  cat <<'EOF' > ~/.zshrc
[ -z "${ITERM_SESSION_ID:0:6}" ] || export SessionID="${ITERM_SESSION_ID:0:6}"

if [ ! -z "$SessionID" ]; then
  if [ -t 0 ] && [[ -z "$TMUX" ]] && [[ $- = *i* ]]; then
    tmux has-session -t $SessionID 2>/dev/null

    if [ $? != 0 ]; then
      tmux new-session -s $SessionID \; setenv SessionID $SessionID
    else
      tmux attach-session -t $SessionID
    fi
    exit 0
  else
    for var in $(tmux show-environment | grep -v "^-"); do eval "export $var"; done > /dev/null 2>&1
    #export GPG_TTY=$(tty)
  fi
fi
...
function myips() {
  ...
  ifconfig en0 | grep 'inet ' | sed -e 's/:/ /' | awk '{print "en0 (IPv4): " $2 " " $3 " " $4}'
  ifconfig en0 | grep 'inet6 ' | sed -e 's/ / /' | awk '{print "en0 (IPv6): " $2 " " $3 " " $4}'
  ifconfig en1 | grep 'inet ' | sed -e 's/:/ /' | awk '{print "en1 (IPv4): " $2 " " $3 " " $4}'
  ifconfig en1 | grep 'inet6 ' | sed -e 's/ / /' | awk '{print "en1 (IPv6): " $2 " " $3 " " $4}'
